...
function TreeNode({ node, relationship, depth }) {
  ...
  const companyShareholders = shareholderLinks.filter(l => l.linkModel === 'Company')
  ...
  return (
    <div className="relative">
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              <Link to={`/companies/${company._id}`} className="font-semibold text-gray-900 hover:text-primary-600 truncate block">{company.name}</Link>
              <p className="text-xs text-gray-400">{company.registrationNumber} · {company.jurisdiction}</p>
            </div>
          </div>
          {relationship && (
            <div className="shrink-0 text-right">
              <span className="inline-block px-2 py-1 bg-green-50 text-green-700 border border-green-200 rounded-lg text-xs font-medium">
                持股 {(relationship.shares || 0).toLocaleString()} {relationship.shareType || '普通'} 股
              </span>
              <p className="text-xs text-gray-400 mt-1">占比 {sharePercent(relationship.shares, totalPaidUp)}</p>
            </div>
          )}
        </div>
        ...
        {personShareholders.length > 0 && (
          <div className="mt-3 pt-3 border-t border-gray-100">
            <p className="text-xs font-medium text-gray-400 mb-1">个人股东</p>
            <div className="flex flex-wrap gap-2">
              {personShareholders.map(l => (
                <Link key={l._id} to={`/personnel/${l.link?._id}`}
                  className="inline-flex items-center gap-1 px-2 py-1 bg-gray-50 border border-gray-200 rounded-lg text-xs hover:bg-gray-100">
                  <Users size={11} /> {l.link?.name || '未知'}
                  <span className="text-gray-400">{(l.shares || 0).toLocaleString()} 股</span>
                </Link>
              ))}
            </div>
          </div>
        ...
        {directorLinks.length > 0 && (
          <div className="mt-3 pt-3 border-t border-gray-100">
            <p className="text-xs font-medium text-gray-400 mb-1">董事 / 公司秘书</p>
            <div className="flex flex-wrap gap-2">
              {directorLinks.map(l => (
                <Link key={l._id} to={`/personnel/${l.link?._id}`}
                  className="inline-flex items-center gap-1 px-2 py-1 bg-blue-50 border border-blue-200 rounded-lg text-xs text-blue-700 hover:bg-blue-100">
                  {l.link?.name || '未知'}
                  <span className="text-blue-400">{(l.roles || []).join('/')}</span>
                </Link>
              ))}
            </div>
          </div>
      ...
      {/* 子节点（公司类股东） */}
      {children.length > 0 && (
        <div className="mt-4 ml-6 space-y-4 border-l-2 border-dashed border-gray-200 pl-6">
          {children.map(child => (
...
export default function EquityGraph({ companyId }) {
  ...
  if (!tree) return <p className="text-gray-400 text-sm">无法加载股权架构</p>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 text-sm text-gray-500">
        ...
        <span>从 <strong className="text-gray-800">{tree.company.name}</strong> 出发，按股东记录向下穿透（最多 {MAX_DEPTH} 层）。点击任意公司/人员可查看详情。</span>
      </div>
