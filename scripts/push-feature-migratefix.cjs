// 追加提交：把 migrate-jurisdiction.js 的 Rule 归一化 bugfix 追加到 feature/jurisdiction-unify 分支。
// 基于「feature 分支当前 HEAD」派生新 commit（追加，不覆盖），只叠加重修的 migrate-jurisdiction.js。
// 用法： $env:GITHUB_TOKEN="..."; node scripts/push-feature-migratefix.cjs
...
const COMMIT_MSG = 'fix: migrate-jurisdiction Rule normalization keeps enum case (Cayman not CAYMAN)';

// 仅这一个 bugfix 文件进入本次追加提交
const TARGET_FILES = ['scripts/migrate-jurisdiction.js'];
